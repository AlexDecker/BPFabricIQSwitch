...
switchCtrlReg* createControlRegisters(){
	...
	ctrl->polling = (bool*)calloc(dataplane.port_count,sizeof(bool));
...
void* commonDataPath(void* arg){
	...
	while (likely(!sigint)) {
			...
			Arg->ctrl->polling[Arg->portNumber] = true;
			poll(Arg->pfd, 1, -1);
			Arg->ctrl->polling[Arg->portNumber] = false;
		
		sched_yield();//TIRAR ESSA DISGRETA DAQUI!!!!!!!!!
	}
	pthread_exit(NULL);
}
